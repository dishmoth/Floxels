...
class SoundEffect implements LineListener {
  ...
  static private void initialize(AudioFormat format) throws IOException {
    ...
    if ( maxLines != AudioSystem.NOT_SPECIFIED ) {
      if ( Env.debugMode() ) {
        System.out.println("Default Sound Mixer: " + kMixerInfo + ", max lines "
      ...
      for ( int k = 0 ; k < mixerInfo.length ; k++ ) {
        ...
        if ( n != 0 && Env.debugMode() ) {
          System.out.println("Sound Mixer " + k + ": " + mixerInfo[k] 
    ...
    if ( Env.debugMode() ) {
      System.out.println("Using Sound Mixer: " + kMixerInfo + ", max lines " 
  ...
  public void play() {
    ...
    if ( mClip == null ) {
      ...
      try {
        ...
        if ( Env.debugMode() ) {
          System.out.println("SoundEffect: could not play: " + ex);
        }
        mData = null;
        mFormat = null;
      }
